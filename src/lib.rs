...
impl SplitMix {
    #[inline(always)]
    fn round(&self, x: u64, add0: u64, add1: u64) -> u64 {
        let z = x.wrapping_add(add0) & self.mask;
        let z = (z ^ (z >> self.s1)).wrapping_mul(C1) & self.mask;
        let z = z.wrapping_add(add1) & self.mask;
...
impl Bijection for SplitMix {
    fn apply(&self, x: u64) -> u64 {
        let z = self.round(x, self.seed0, self.seed1);
        self.round(z, self.seed0.rotate_right(32), self.seed1.rotate_right(32))
    }
}
